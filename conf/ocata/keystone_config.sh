    ...
    for service in $SERVICES; do
        ...
        if [ $? -ne 0 ]; then
            if [ $service == 'keystone' ] ; then
                ...
                openstack endpoint create --region $REGION placement public http://$CTRL_MGMT_IP:8780
                openstack endpoint create --region $REGION placement internal http://$CTRL_MGMT_IP:8780
                openstack endpoint create --region $REGION placement admin http://$CTRL_MGMT_IP:8780
